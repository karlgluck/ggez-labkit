...
namespace GGEZ
{
    ...
    */
    ...
    public class ECSController : MonoBehaviour
    {
        ...
        private Dictionary<Type, ArrayList> _componentsByType = new Dictionary<Type, ArrayList>();
        private ArrayList _entitiesCreatedDuringIteration = new ArrayList();
        private ArrayList _componentsAcquiredDuringIteration = new ArrayList();
        private bool _anyComponentsWereRemoved;
        ...
        {
            ArrayList retval;
            if (!_componentsByType.TryGetValue(type, out retval))
            {
                retval = new ArrayList();
                _componentsByType[type] = retval;
            }
            return retval;
        }
        ...
        {
            foreach (var kv in _componentsByType)
            {
                kv.Value.RemoveNullElementsStable();
            }
            _anyComponentsWereRemoved = false;
        }
        ...
        public void ForEachEntity(Delegate callback)
        {
            ...
            if (parameters.Length > _allComponentsOfParameterType.Length)
            {
                throw new InvalidOperationException("Too many components used in callback");
            }
            // Debug.LogFormat ("-------------------------------------------------");
            var current = _current;
            var components = _components[parameters.Length];
            var allComponentsOfParameterType = _allComponentsOfParameterType;
            int _sentinel = 0;
            // Debug.LogFormat ("---------------");
...
#pragma warning restore 0168
            ...
            for (int i = 0; i < _entitiesCreatedDuringIteration.Count; ++i)
            {
                var entity = (ECSEntity)_entitiesCreatedDuringIteration[i];
                int j = 0;
            ...
            _entitiesCreatedDuringIteration.Clear();

            for (int i = 0; i < _componentsAcquiredDuringIteration.Count; ++i)
            {
                var componentAcquiredDuringIteration = (ECSBaseComponent)_componentsAcquiredDuringIteration[i];
            ...
            _componentsAcquiredDuringIteration.Clear();

            if (_entitiesCreatedDuringIteration.Count > 0)
            {
