...
//-----------------------------------------------------------------------------
// OmnibusEditorUtility
//-----------------------------------------------------------------------------
public static class OmnibusEditorUtility
{
    ...
    // outputLabelStyle
    //-----------------------------------------------------
    private static GUIStyle _outputLabelStyle;
    public static GUIStyle outputLabelStyle
    {
        get
        {
            if (_outputLabelStyle == null)
            {
                _outputLabelStyle = new GUIStyle(EditorStyles.label);
                _outputLabelStyle.alignment = TextAnchor.MiddleRight;
            }
            return _outputLabelStyle;
        }
    }
    ...
    // cellNameStyle
    //-----------------------------------------------------
    private static GUIStyle _cellNameStyle;
    public static GUIStyle cellNameStyle
    {
        get
        {
            if (_cellNameStyle == null)
            {
                _cellNameStyle = new GUIStyle(EditorStyles.label);
                _cellNameStyle.alignment = TextAnchor.MiddleCenter;
                _cellNameStyle.fontStyle = FontStyle.Bold;
            }
            return _cellNameStyle;
        }
    }
    ...
    private static GUIStyle[] _nodeStyles;
    ...
    private static GUIStyle[] _nodeStylesSelected;
    public static GUIStyle[] NodeStylesSelected
    {
        get
        {
            if (_nodeStylesSelected == null)
            {
                _nodeStylesSelected = new GUIStyle[7];
                for (int i = 0; i < _nodeStylesSelected.Length; ++i)
                {
                    var style = new GUIStyle("flow node " + i + " on");
                    style.padding = new RectOffset (0,0,0,0);
                    style.margin = new RectOffset (0,0,0,0);
                    _nodeStylesSelected[i] = style;
                }
            }
            return _nodeStylesSelected;
        }
    }
    ...
    public static Texture2D GridTexture
    {
        get
        {
            ...
            {
                _gridTexture = OmnibusEditorUtility.GenerateGridTexture (GridSize*10, GridSize, new Color(0.9f,0.9f,0.9f), new Color(0.8f,0.8f,0.8f));
            }
            return _gridTexture;
        }
    }
    ...
    public static Texture2D SelectedBackgroundTexture
    {
        get
        {
            ...
            {
                _selectedBackgroundTexture = OmnibusEditorUtility.GenerateSolidColorTexture (64, SelectedColor);
            }
    ...
    public static Rect BeginNode (string name, Rect position, bool selected, Vector2 anchor, NodeStyleColor color = NodeStyleColor.Gray)
    {
        ...
        EditorGUI.LabelField(titleRect, name, OmnibusEditorUtility.cellNameStyle);
    ...
    public static void DrawPort (Vector2 position, bool filled, bool selected)
    {
        ...
        Handles.color = selected ? OmnibusEditorUtility.SelectedColor : OmnibusEditorUtility.WireColor;
    ...
    {
        var fromCenter = fromState.Position.center - Vector2.down * OmnibusEditorUtility.NodeTitleRectHeight * 0.5f;
        var toCenter = toState.Position.center - Vector2.down * OmnibusEditorUtility.NodeTitleRectHeight * 0.5f;
    ...
    public static void DrawTransition(EditorTransition editorTransition, EditorState fromState, EditorState toState, bool hasReverse, bool selected, Vector2 anchor)
    {
        ...
        OmnibusEditorUtility.DrawExpressionSquiggle(
            from,
            to,
        ...
        OmnibusEditorUtility.UpdateExpressionPositions(editorTransition, flipTreeSide);
        OmnibusEditorUtility.DrawTransitionExpression(anchor, editorTransition.Expression, flipTreeSide);
    ...
    const float typeLabelWidth = 65f;
    static float typeLabelHeight { get { return EditorGUIUtility.singleLineHeight; } }
    const float spaceBetweenLabelAndSubexpressions = 10f;
...
{
    private static Dictionary<Type,InspectableType> typeToInspectableType;
    public static InspectableType GetInspectableTypeOf(Type type)
    {

        // If we haven't built our type-map for the enum yet, do it now
        if (typeToInspectableType == null)
        {
            typeToInspectableType = new Dictionary<Type, InspectableType>();
            ...
            for (int i = 0; i < enumValues.Length; ++i)
            {
                ...
                {
                    typeToInspectableType.Add(attribute.Type, value);
                }
            }
        }
        ...
        if (typeToInspectableType.TryGetValue(type, out retval))
        {
            return retval;
        }

        // Try inheritance
        foreach (var kvp in typeToInspectableType)
        {
...
public class InspectableCellType
{
    ...
    public static InspectableCellType GetInspectableCellType(Type cellType)
    {
        ...
        {
            var portCenter = new Vector2(OmnibusEditorUtility.GridSize - OmnibusEditorUtility.NodeLeftRightMargin, EditorGUIUtility.singleLineHeight * (0.5f + i));
