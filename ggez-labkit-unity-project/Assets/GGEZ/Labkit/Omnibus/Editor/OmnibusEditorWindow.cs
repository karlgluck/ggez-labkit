...
namespace GGEZ.Omnibus
{
    public class OmnibusEditorWindow : EditorWindow
    {
        ...
        private void OnGUI()
        {
            ...
            if (IsCreatingWire)
            {
                OmnibusEditorUtility.DrawBezier(_scrollPosition + _scrollAnchor + _creatingWireStartPoint, _scrollPosition + _scrollAnchor + _creatingWireEndPoint, Vector2.zero, Vector2.zero, false);
                OmnibusEditorUtility.DrawPort(_scrollPosition + _scrollAnchor + _creatingWireStartPoint, true, false);
                if (CreatingWireHoveringEndPoint)
                {
                    OmnibusEditorUtility.DrawPort(_scrollPosition + _scrollAnchor + _creatingWireEndPoint, true, false);
                }
                ...
                if (Event.current.type == EventType.MouseDrag || (Event.current.type == EventType.MouseUp && Event.current.button == 0))
                {
                    if (IsCreatingInputWire && pickEditorCellOutputPort(mouseGraphPosition, out _creatingWireEndEditorCell, out _creatingWireEndInspectableType, out _creatingWireEndPort))
                    {
                        ...
                        {
                            var endPoint = OmnibusEditorUtility.GetNodeOutputPortRect(_creatingWireEndEditorCell.Position, _creatingWireEndInspectableType.Outputs[_creatingWireEndPort].PortCenterFromTopRight).center;
                    ...
                    else if (!IsCreatingInputWire && pickEditorCellInputPort(mouseGraphPosition, out _creatingWireEndEditorCell, out _creatingWireEndInspectableType, out _creatingWireEndPort))
                    {
                        ...
                        if (Event.current.type == EventType.MouseDrag)
                        {
                            var endPoint = OmnibusEditorUtility.GetNodeInputPortRect(_creatingWireEndEditorCell.Position, _creatingWireEndInspectableType.Inputs[_creatingWireEndPort].PortCenterFromTopLeft).center;
                            if (!CreatingWireHoveringEndPoint || _creatingWireEndPoint != endPoint)
                            {
                                CreatingWireHoveringEndPoint = true;
                                _creatingWireEndPoint = endPoint;
