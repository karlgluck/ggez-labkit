...
#endif
    this.targetAlpha = (this.invert == (bool)value) ? 0f : 1f;
    this.enabled = true;
    }
...
void Update ()
    {
    var color = this.image.color;
	var alpha = Mathf.MoveTowards (color.a, this.targetAlpha, Time.smoothDeltaTime / this.fadeTime);
	this.enabled = !Mathf.Approximately (color.a, alpha);
	color.a = alpha;
	this.image.color = color;
    }
