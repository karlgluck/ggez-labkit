...
[
Serializable,
CreateAssetMenu (fileName = "New Int Table Register.asset", menuName="GGEZ/Game Register/Table/int Table Register")
]
public class IntTableRegister : GameTableRegister, ISerializationCallbackReceiver
{
...
[SerializeField] private List<IntTableRegisterKeyValuePair> initialTable = new List<IntTableRegisterKeyValuePair> ();

// Values that the table has at runtime only. Entries are cleared out when
// listeners unregister.
[SerializeField] private List<IntTableRegisterKeyValuePair> runtimeTable = new List<IntTableRegisterKeyValuePair> ();
...
void ISerializationCallbackReceiver.OnBeforeSerialize ()
    ...
    foreach (var key in runtimeKeys)
        {
        this.runtimeTable.Add (new IntTableRegisterKeyValuePair (key, this.table[key]));
        }
    }
...
void ISerializationCallbackReceiver.OnAfterDeserialize ()
    ...
    foreach (var key in deletedKeys)
        {
        this.runtimeTable.Add (new IntTableRegisterKeyValuePair (key, this[key]));
        }
...
void Reset ()
    ...
    this.initialTable = new List<IntTableRegisterKeyValuePair> ();
    this.runtimeTable = new List<IntTableRegisterKeyValuePair> ();
    this.dirtyKeys.Clear ();
...
void OnValidate ()
    ...
    if (Application.isPlaying)
        ...
        foreach (var key in this.dirtyKeys)
            {
            List<IntTableRegisterListener> listeners;
...
private Dictionary<string,List<IntTableRegisterListener>> listenersTable = new Dictionary<string,List<IntTableRegisterListener>>();





public void RegisterListener (string key, IntTableRegisterListener listener)
    {
    if (key == null)
        {
    ...
    if (!this.listenersTable.TryGetValue (key, out listeners))
        {
        listeners = new List<IntTableRegisterListener> ();
...
public void UnregisterListener (string key, IntTableRegisterListener listener)
    {
    if (key == null)
        {
        throw new ArgumentNullException ("key");
...
//----------------------------------------------------------------------
[System.Serializable]
public class IntTableRegisterKeyValuePair
{
...
public IntTableRegisterKeyValuePair ()
    {
    }

public IntTableRegisterKeyValuePair (string key, int value)
    {
    this.Name = key;
    this.Value = value;
    }
