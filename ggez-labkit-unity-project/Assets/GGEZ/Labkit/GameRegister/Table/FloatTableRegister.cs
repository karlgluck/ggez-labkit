...
[
Serializable,
CreateAssetMenu (fileName = "New Float Table Register.asset", menuName="GGEZ/Game Register/Table/float Table Register")
]
public class FloatTableRegister : GameTableRegister, ISerializationCallbackReceiver
{
...
[SerializeField] private List<FloatTableRegisterKeyValuePair> initialTable = new List<FloatTableRegisterKeyValuePair> ();

// Values that the table has at runtime only. Entries are cleared out when
// listeners unregister.
[SerializeField] private List<FloatTableRegisterKeyValuePair> runtimeTable = new List<FloatTableRegisterKeyValuePair> ();
...
void ISerializationCallbackReceiver.OnBeforeSerialize ()
    ...
    foreach (var key in runtimeKeys)
        {
        this.runtimeTable.Add (new FloatTableRegisterKeyValuePair (key, this.table[key]));
        }
    }
...
void ISerializationCallbackReceiver.OnAfterDeserialize ()
    ...
    foreach (var key in deletedKeys)
        {
        this.runtimeTable.Add (new FloatTableRegisterKeyValuePair (key, this[key]));
        }
...
void Reset ()
    ...
    this.initialTable = new List<FloatTableRegisterKeyValuePair> ();
    this.runtimeTable = new List<FloatTableRegisterKeyValuePair> ();
    this.dirtyKeys.Clear ();
...
void OnValidate ()
    ...
    if (Application.isPlaying)
        ...
        foreach (var key in this.dirtyKeys)
            {
            List<FloatTableRegisterListener> listeners;
...
private Dictionary<string,List<FloatTableRegisterListener>> listenersTable = new Dictionary<string,List<FloatTableRegisterListener>>();





public void RegisterListener (string key, FloatTableRegisterListener listener)
    {
    if (key == null)
        {
    ...
    if (!this.listenersTable.TryGetValue (key, out listeners))
        {
        listeners = new List<FloatTableRegisterListener> ();
...
public void UnregisterListener (string key, FloatTableRegisterListener listener)
    {
    if (key == null)
        {
        throw new ArgumentNullException ("key");
...
//----------------------------------------------------------------------
[System.Serializable]
public class FloatTableRegisterKeyValuePair
{
...
public FloatTableRegisterKeyValuePair ()
    {
    }

public FloatTableRegisterKeyValuePair (string key, float value)
    {
    this.Name = key;
    this.Value = value;
    }
