...
namespace GGEZ.FullSerializer {
    public class fsSerializer {
        ...
        /// </summary>
        private static readonly string Key_UnityObjectReference = string.Format("{0}unity", fsGlobalConfig.InternalFieldPrefix);
        ...
        public static void StripDeserializationMetadata(ref fsData data) {
            ...
            if (data.IsDictionary) {
                ...
                dict.Remove(Key_UnityObjectReference);
        ...
        /// <summary>
        /// Reference manager for Unity objects.
        /// </summary>
        ...
        public fsResult TrySerialize(Type storageType, Type overrideConverterType, object instance, out fsData data) {
            ...
            {
                data = fsData.CreateDictionary();
                data.AsDictionary[Key_UnityObjectReference] = new fsData(_unityReferences.GetReferenceId(instance));
                Invoke_OnAfterSerialize(processors, storageType, instance, ref data);
                return fsResult.Success;
            }
        ...
        private fsResult InternalDeserialize_3_Inheritance(Type overrideConverterType, fsData data, Type storageType, ref object result, out List<fsObjectProcessor> processors) {
            ...
            if (IsTypeSpecified(data)) {
                ...
                do {
                    ...
                    if (type == null) {
                        // This was changed in May 2016 to be an error. However, unit tests were not updated
                        // and they still expect this to fail gracefully. Given how tolerant the surrounding
                        // code is, I don't think that failing is consistent behavior and the commit doesn't
                        // justify why it would be. I demoted this back to a warning. - kg
                        // https://github.com/jacobdufault/fullserializer/commit/52d9f02a83dd1690176c10c9422a682f2bb000d7#diff-6a3b8f8a3dd014ccd3fc6c508403d8d2
                        deserializeResult += fsResult.Warn("Unable to locate specified type \"" + typeName + "\"");
                        break;
                    }
