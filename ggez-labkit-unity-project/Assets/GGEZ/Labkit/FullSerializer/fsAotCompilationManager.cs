...
namespace GGEZ.FullSerializer
{
    ...
    public class fsAotCompilationManager
    {
        ...
        public static Dictionary<Type, string> AvailableAotCompilations
        {
            get
            {
                for (int i = 0; i < s_uncomputedAotCompilations.Count; ++i)
                {
                    var item = s_uncomputedAotCompilations[i];
                    s_computedAotCompilations[item.Type] = GenerateDirectConverterForTypeInCSharp(item.Type, item.Members, item.IsConstructorPublic);
                }
                s_uncomputedAotCompilations.Clear();

                return s_computedAotCompilations;
            }
        }
        private static Dictionary<Type, string> s_computedAotCompilations = new Dictionary<Type, string>();
        ...
        private static List<AotCompilation> s_uncomputedAotCompilations = new List<AotCompilation>();
        ...
        {
            s_uncomputedAotCompilations.Add(new AotCompilation
            {
                Type = type,
                Members = members,
